...
def extract_text(content) -> str:
    """Normalize an LLM message's .content into plain text.

    Newer langchain-google-genai versions return content as a list of
    structured blocks (e.g. [{"type": "text", "text": "..."}]) instead of
    a plain string. Flatten that here so callers always get a str.
    """
...
def rewrite_query(state: AgentState):
    rewritten = extract_text(llm().invoke(f"""
...
def direct_answer(state: AgentState):
    answer = extract_text(llm().invoke(f"Respond briefly and naturally to: {state['question']}").content)
